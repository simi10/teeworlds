...
int CMenus::RenderMenubar(CUIRect r)
{
	...
	if(Client()->State() == IClient::STATE_OFFLINE)
	{
		...
		if((DoButton_MenuTab(&s_InternetButton, Localize("Internet"), m_ActivePage==PAGE_INTERNET, &Button, CUI::CORNER_TL|CUI::CORNER_IBL) || CheckHotKey(KEY_i)) && m_ActivePage!=PAGE_INTERNET)
		{
		...
		else if(m_MenuPage == PAGE_SETTINGS)
		{
			...
			if(DoButton_MenuTabTop(&s_ChatButton, Localize("Chat"), g_Config.m_UiSettingsPage==SETTINGS_CHAT, &Button, CUI::CORNER_T))
			{
				g_Config.m_UiSettingsPage = SETTINGS_CHAT;
			}

			Box.VSplitLeft(100.0f, &Button, &Box);
			static int s_HudButton=0;
			if(DoButton_MenuTabTop(&s_HudButton, Localize("Hud"), g_Config.m_UiSettingsPage==SETTINGS_HUD, &Button, CUI::CORNER_T))
			{
				g_Config.m_UiSettingsPage = SETTINGS_HUD;
			}

			Box.VSplitLeft(100.0f, &Button, &Box);
			static int s_RaceButton=0;
			if(DoButton_MenuTabTop(&s_RaceButton, Localize("Race"), g_Config.m_UiSettingsPage==SETTINGS_RACE, &Button, CUI::CORNER_T))
			{
				g_Config.m_UiSettingsPage = SETTINGS_RACE;
			}

			Box.VSplitLeft(100.0f, &Button, &Box);
			static int s_TeecompButton=0;
			if(DoButton_MenuTabTop(&s_TeecompButton, "Teecomp", g_Config.m_UiSettingsPage==SETTINGS_TEECOMP, &Button, CUI::CORNER_T))
			{
	...
	else
	{
		...
		if(m_GamePage != PAGE_SETTINGS) // Game stuff
		{
			...
			if(DoButton_MenuTab(&s_GameButton, Localize("Game"), m_ActivePage==PAGE_GAME, &Button, CUI::CORNER_TL|CUI::CORNER_IBL) || CheckHotKey(KEY_g))
				NewPage = PAGE_GAME;
			...
			if(DoButton_MenuTab(&s_QuitButton, Localize("Quit"), 0, &Button, CUI::CORNER_TR|CUI::CORNER_IBR) || CheckHotKey(KEY_q))
				m_Popup = POPUP_QUIT;
		...
		else // settings menu
		{
			...
			if(DoButton_MenuTab(&s_InGameSoundButton, Localize("Sound"), g_Config.m_UiSettingsPage==SETTINGS_SOUND, &Button, 0))
				g_Config.m_UiSettingsPage = SETTINGS_SOUND;

			Box.VSplitLeft(90.0f, &Button, &Box);
			static int s_InGameChatButton=0;
			if(DoButton_MenuTab(&s_InGameChatButton, Localize("Chat"), g_Config.m_UiSettingsPage==SETTINGS_CHAT, &Button, 0))
				g_Config.m_UiSettingsPage = SETTINGS_CHAT;

			Box.VSplitLeft(90.0f, &Button, &Box);
			static int s_InHudSoundButton=0;
			if(DoButton_MenuTab(&s_InHudSoundButton, Localize("Hud"), g_Config.m_UiSettingsPage==SETTINGS_HUD, &Button, 0))
				g_Config.m_UiSettingsPage = SETTINGS_HUD;

			Box.VSplitLeft(90.0f, &Button, &Box);
			static int s_InGameRaceButton=0;
			if(DoButton_MenuTab(&s_InGameRaceButton, Localize("Race"), g_Config.m_UiSettingsPage==SETTINGS_RACE, &Button, 0))
				g_Config.m_UiSettingsPage = SETTINGS_RACE;

			Box.VSplitLeft(90.0f, &Button, &Box);
			static int s_InGameTeecompButton=0;
			if(DoButton_MenuTab(&s_InGameTeecompButton, "Teecomp", g_Config.m_UiSettingsPage==SETTINGS_TEECOMP, &Button, CUI::CORNER_TR))
				g_Config.m_UiSettingsPage = SETTINGS_TEECOMP;
