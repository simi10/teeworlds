...
void CSpectator::OnRender()
{
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
			...
			m_pClient->m_Snap.m_pGameDataFlag->m_FlagCarrierBlue == i))
		{
			...
			if(g_Config.m_TcColoredFlags)
			{
				vec3 Col = CTeecompUtils::GetTeamColor(1-m_pClient->m_aClients[i].m_Team, m_pClient->m_Snap.m_pLocalInfo ? m_pClient->m_aClients[m_pClient->m_LocalClientID].m_Team : TEAM_RED, g_Config.m_TcColoredTeesMethod);
